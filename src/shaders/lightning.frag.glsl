...
void main()
{
	vec2 uv = 2.2 * ((gl_FragCoord.xy-ubo.Point) - .5 * ubo.Resolution.xy) / ubo.Resolution.y;

	// use this  to debug in slow motion :^)
	float my_iTime = ubo.Time;    
	...
	float r = .03;
	float a = radians(ubo.Angle) + radians(45.) * (2. * rand(vec2(t_base, 13.)) - 1.);
	...
	for (int i = 0; i < 12; ++i) {
		...
		if (i == 3) {
			...
			branch_1_r = .5 * r;
			branch_1_a = radians(ubo.Angle) + radians(90.) * (2. * rand(vec2(t_base, 13.)) - 1.);
		}
		...
		if (i == 6) {
			...
			branch_2_r = .5 * r;
			branch_2_a = radians(ubo.Angle) - radians(90.) * (2. * rand(vec2(t_base, 17.)) - 1.);
		}

		float len = .12 + .18 * rand(vec2(300. + float(i) + t_base, 1337.));
		float a_next = mix(a, radians(ubo.Angle) + radians(40.) * (2. * rand(vec2(float(300 * i) + t_base, 1337.)) - 1.), .9);
	...
	for (int i = 0; i < 10; ++i) {
		...
		float a_next = mix(radians(ubo.Angle), branch_1_a + radians(40.) * (2. * rand(vec2(float(300 * i) + t_base, 1337.)) - 1.), .5);
	...
	for (int i = 0; i < 10; ++i) {
		...
		float len = .07 + .07 * rand(vec2(300. + float(i) + t_base, 1337.));
		float a_next = mix(radians(ubo.Angle), branch_2_a + radians(40.) * (2. * rand(vec2(float(300 * i) + t_base, 1337.)) - 1.), .5);
	...
	fragColor = vec4(sky
			...
			+ vec3(smoothstep(.0, -.01, d)), ubo.Blend);
